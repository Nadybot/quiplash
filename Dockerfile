...
RUN rm -f target/$RUST_TARGET/release/deps/quiplash*
COPY ./src ./src
COPY ./data ./data
...
RUN source $HOME/.cargo/env && \
    cargo build --release \
        --target="$RUST_TARGET" && \
    cp target/$RUST_TARGET/release/quiplash /quiplash && \
    actual-strip /quiplash
...
COPY --from=builder /quiplash /quiplash
